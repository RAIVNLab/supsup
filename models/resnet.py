...
class ResNet(nn.Module):
    ...
    def _make_layer(self, builder, block, planes, blocks, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(
                block(builder, self.inplanes, planes, base_width=self.base_width)
            )
...
def ResNet18(pretrained=False):
    return ResNet(BasicBlock, [2, 2, 2, 2], args.output_size)


def ResNet50(pretrained=False):
    return ResNet(Bottleneck, [3, 4, 6, 3], args.output_size)


def ResNet101(pretrained=False):
    return ResNet(Bottleneck, [3, 4, 23, 3], args.output_size)


def WideResNet50_2(pretrained=False):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes=1000, base_width=64 * 2)


def WideResNet101_2(pretrained=False):
    return ResNet(Bottleneck, [3, 4, 23, 3], num_classes=1000, base_width=64 * 2)
