        ...
        if args.reinit_adapt == "n-1-imagenet":
            ...
            for n, m in model.named_modules():
                if isinstance(m, nn.Conv2d):
                    if args.reinit_adapt == "running_mean":
                        m.scores[task_idx].data = (
                            m.stacked[:task_idx].clone()
                            / m.scores[:task_idx].norm()
                        )
                    else:
                        m.scores[task_idx].data = m.stacked[
                            predicted_similar_task
                        ].data / m.scores[predicted_similar_task].norm()
